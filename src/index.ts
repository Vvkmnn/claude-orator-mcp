...
server.registerTool(
  'orator_optimize',
  {
    ...
    inputSchema: {
      ...
      intent: OptimizeInputSchema.shape.intent.describe(
        'Intent category (auto-detected if omitted)',
      ),
      target: OptimizeInputSchema.shape.target.describe(
        'Target environment for the optimized prompt (default: claude-code)',
      ),
      techniques: OptimizeInputSchema.shape.techniques.describe(
  ...
  async (args) => {
    try {
      ...
      if (result.analysis?.semantic_hints.length && clientSupportsSampling(server.server)) {
        const validation = await validateViaSampling(
          server.server,
          result,
          result.analysis.semantic_hints,
        );
